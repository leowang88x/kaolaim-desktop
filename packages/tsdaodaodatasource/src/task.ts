...
export class MediaMessageUploadTask extends MessageTask {
    ...
    async start(): Promise<void> {
        ...
        if (mediaContent.contentType === 5) {
            // 视频上传  先上传封面，在上传视频
            this.getUploadFileUrlCover();
        } else {
            // 其他文件上传
            this.fileUpload();
        }
    ...
    async getUploadFileUrlCover() {
        const mediaContent = this.message.content as MediaMessageContent
        if (mediaContent.coverFile) {
            let tempFileName = mediaContent.coverFile.name;
            let prefix = tempFileName.substring(tempFileName.lastIndexOf('.') + 1);
            const fileName = this.getUUID();
            // 构建文件路径
            let path = `/${this.message.channel.channelType}/${this.message.channel.channelID}/${fileName}.${prefix}`;
            // 构建请求的 URL
            const uploadURL = await this.getUploadURL(path)
            if (uploadURL) {
                this.uploadFileCover(mediaContent.coverFile, uploadURL)
            } else {
    ...
    async getUploadFileUrlVideo() {
        // 获取文件扩展名
        const mediaContent = this.message.content as MediaMessageContent
        if (mediaContent.file) {
            let tempFileName = mediaContent.file.name;
            let prefix = tempFileName.substring(tempFileName.lastIndexOf('.') + 1);
            const fileName = this.getUUID();
            // 构建文件路径
            let path = `/${this.message.channel.channelType}/${this.message.channel.channelID}/${fileName}.${prefix}`;
            ...
            const uploadURL = await this.getUploadURL(path)
            if (uploadURL) {
                this.uploadFileVideo(mediaContent.file, uploadURL)
            } else {
                console.log('获取上传地址失败！')
                this.status = TaskStatus.fail
                this.update()
    ...
    async uploadFileCover(file: File, uploadURL: string) {
        const param = new FormData();
        param.append("file", file);
        const resp = await axios.post(uploadURL, param, {
            headers: { "Content-Type": "multipart/form-data" },
        ...
        if (resp) {
            if (resp.data.path) {
                const mediaContent = this.message.content as MediaMessageContent
                mediaContent.coverUrl = resp.data.path
                await this.getUploadFileUrlVideo();
                // this.status = TaskStatus.success
                // this.update()
            }
        }
    ...
    async uploadFileVideo(file: File, uploadURL: string) {
        const param = new FormData();
        param.append("file", file);
        const resp = await axios.post(uploadURL, param, {
            headers: { "Content-Type": "multipart/form-data" },
    ...
    async getUploadURL(path: string): Promise<string | undefined> {
        const result = await WKApp.apiClient.get(`file/upload?type=chat&path=${path}`)
        if (result) {
            return result.url
        }
    }
